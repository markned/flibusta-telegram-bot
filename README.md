...
- переход из карточки к остальным книгам надёжно распознанной серии;
- поиск и сортировка внутри избранного в веб-версии;
...
`/admin_health` показывает локальную диагностику SQLite, WAL, свободного места, кэша, очереди, веб-сессий и агрегированных промахов поиска. Команда не обращается к внешней сети и не выводит запросы, e-mail или секреты.
...
- вежливые вводные фразы вроде «пожалуйста, найди» удаляются до точного поиска, а статистика пустых результатов хранится только агрегатами без текста запроса;

## Резервные копии SQLite

Создать проверенный снимок базы без прямого копирования WAL-файла:

```bash
python scripts/backup_sqlite.py --database bot.db --output-dir backups --keep 14
```

Скрипт использует SQLite backup API, выполняет `PRAGMA integrity_check`, создаёт файлы с правами `0600` и удаляет старые снимки сверх лимита. Каталог `backups/` не должен публиковаться или попадать в образ приложения.
