...
Интерфейс не требует JavaScript, определяет Kindle/PocketBook по `User-Agent` и использует совместимую с их старыми браузерами линейную вёрстку. На e-ink карточках обложки не загружаются: это исключает битые изображения и ускоряет страницу. Сырые коды и сессионные токены не сохраняются: SQLite содержит только HMAC-хеши. Книги передаются по запросу и не хранятся постоянно.
